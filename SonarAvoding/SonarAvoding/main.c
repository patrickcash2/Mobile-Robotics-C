...
#define DEG_90  130     /* Number of steps for a 90-degree (in place) turn. */
...
do {                                  \
	...
	do {                                 \
		...
		void homing( volatile MOTOR_ACTION *pAction, volatile SENSOR_DATA *pSensor );
		...
		void info_display( volatile MOTOR_ACTION *pAction, volatile SENSOR_DATA *pSensor )
		{
			...
			if ( ( pAction->state != previous_state ) || ( pAction->state == STARTUP ) )
			{
				...
				switch( pAction->state )
				{
					...
					LCD_printf("left: %.3f right: %.3f dist: %.3f", pSensor->left_PH, pSensor->right_PH, (pSensor->dist_SONAR) );
					...
					LCD_printf( "Homing...\n" );
					LCD_printf("left: %.3f right: %.3f", pSensor->left_PH, pSensor->right_PH);
		...
		void homing( volatile MOTOR_ACTION *pAction, volatile SENSOR_DATA *pSensor )
		{
			if( (pSensor->left_PH + pSensor->right_PH) > 4 )	// WARNING: this may be too high for dark room
			{
